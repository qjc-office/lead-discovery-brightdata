...
소스
  ...
  wanted   원티드 공개 채용 API. robots.txt 요청이 CDN에서 403으로 끊긴다.
           브라우저 UA·봇 UA·UA 없음 전부 403이라 우리 쪽 UA 때문이 아니라
           경로가 일괄로 막힌 상태다. RFC 9309 §2.3.1.3 은 이 경우(400~499)를
           "unavailable" 로 보고 크롤러가 접근해도 된다고 규정하지만, 규정이
           허용한다고 마음껏 긁을 이유는 없어서 요청 간격을 두고 목록 페이지만
           최소로 가져온다. robots.txt 판정이 이용약관 검토를 대신하지는 않는다.
